    ...
    public class CreateGrid
    {
            ...
            Vector2Int canvasOriginConverted)
        {
            ...
            Vector2Int upperRight = cellGridScale * new Vector2Int(row + 1, col + 1) + canvasOriginConverted +
                                    new Vector2Int(-shrinkAmount, -shrinkAmount);
            return new[] { upperRight, lowerRight, lowerLeft, upperLeft };
        }

        private static bool IsCellBlocked(Vector2Int[] cellGrid, List<Polygon> obstacles)
        {
            Vector2Int minCellBounds = cellGrid[2]; // lowerLeft
            Vector2Int maxCellBounds = cellGrid[0]; // upperRight

            foreach (Polygon obstacle in obstacles)
            {
                Vector2Int[] polygon = obstacle.getIntegerPoints();

                // Check if any point of the polygon is inside the cell grid
                foreach (Vector2Int point in polygon)
                {
                    if (IsPointInsideAxisAlignedBoundingBox(minCellBounds, maxCellBounds, point))
                    {
                        return true;
                    }
                }

                // Check if any point of the cell grid is inside the polygon
                foreach (Vector2Int point in cellGrid)
                {
                ...
                // Check if any edge of the polygon intersects with any edge of the cell grid
